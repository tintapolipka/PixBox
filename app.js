...
//function for setting the tilenumber:
const setX = (plusorminus1)=>{
  if(rowLengthInTile<2){rowLengthInTile=2}
  if(plusorminus1<0){ rowLengthInTile--; } 
  else {rowLengthInTile++};
  document.getElementById('valueX').innerText = rowLengthInTile;
}

const setY = (plusorminus1)=>{
  if(columnLengthInTile<2){columnLengthInTile=2}
  if(plusorminus1<0){ columnLengthInTile--; } 
  else {columnLengthInTile++};
  document.getElementById('valueY').innerText = columnLengthInTile;
}
...
const smallPic = ()=>{
  ...
  smp.innerHTML= '';
  smp.innerHTML= `<style>#smallPic{width:${10*rowLengthInTile}px; height:${10*columnLengthInTile}px;}</style>`
...
let tileSide = 40;
if(rowLengthInTile>15){tileSide=25};
...
for(i=1; i<((columnLengthInTile*rowLengthInTile)+1); i++){
  if(!loadingTrue){pixDataArr[i] = new TileObj(i,'none');}
  finalTxt +=`<div id="tile${i}" class="tileBasicClass class${i}" onclick="coloringTiles(${i})">
  <style> .tileBasicClass{width:${100/rowLengthInTile}%; height:${100/columnLengthInTile}%;} #tile${i} {background-color:`
     if(loadingTrue){finalTxt += pixDataArr[i].color
      }
      else {finalTxt +='rgba(1,1,1,0)'}; 
...
const undo = ()=>{
    ...
    if(colorHistory[0].length===0){console.log('No more actions to undo.')} 
    else {
...
function TileObj(arrindex,color) {this.positionX = ((arrindex%rowLengthInTile === 0 ? (rowLengthInTile-1) : arrindex%rowLengthInTile-1))*10; this.positionY = (Math.trunc(arrindex/rowLengthInTile)=== arrindex/rowLengthInTile ? (arrindex/rowLengthInTile)-1 : Math.trunc(arrindex/rowLengthInTile))*10; this.color = color;}
...
//Színválasztás:
  ...
  function Colour(name,code){this.name=name; this.code=code;}


 document.getElementById('newPicActivator').addEventListener('click',function(e){document.getElementById('newPic').classList.toggle('left_toggle');
    if(e.target.innerText ==='NEW'){
    e.target.innerText ='Hide'} else {e.target.innerText ='NEW'}
    })
...
function bgImported(){
  ...
  document.getElementById('tileDivSection').style.background = 
`url(${x}) center center/cover`;

  }
  else if(document.getElementById('bgImgUrl').value){
    let y = document.getElementById('bgImgUrl').value;
    document.getElementById('tileDivSection').style.background = 
  `url(${y}) center center/cover`;} else {console.log('No file for background now')}
}
...
const colorExtractor = ()=>{
    ...
    return noneLessArr.map(item=>{if(item==='none'){return 'rgba(1,1,1,0)'}else {return item}})
}
...
const loader =()=>{
  const [file] = document.querySelector('#inputSVGText').files;
  if(file){
  alert('Your current work will be lost. If you want to save it, download SVG now, then click Load button again!')  
...
makePixDataArr();
newPicGenerator(document.querySelector('input.palselect:checked').value,true);
})
   
} else {alert('No SVG file selected. Use fileselector to select one.')}

};

// loader() és után a lehet ;
